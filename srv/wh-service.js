...
module.exports = function (srv) {
  const { WarehouseOrderItems, WarehouseOrders, DeliveryForecasts, Orders } = srv.entities;

  const repeatIntervalCronPattern = process.env.SCHEDULER_REPEAT_PATTERN || '* * * * *'; // Every minute
  ...
  this.on(["updateDelivery", "triggerDeliveryUpdate"], async (req) => {
    const data = await SELECT.from(WarehouseOrders, (whOrder) => {
      whOrder`.*`, whOrder.parentOrder((pO) => {
    ...
    if (parentOrdersID.length) {
      const parentOrders = await SELECT.from(Orders, (order) => {
        order`.*`, order.warehouseOrders((whO) => {
          whO`.*`
        })
      }).where({
        ID: {
